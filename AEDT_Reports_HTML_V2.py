...
This example script is written mainly in Cpython but has AEDT (Ansys Electronics Desktop) IronPython classes.

It can be run directly in Cpython environment.

If this script is run in Spyder, we need to make this change:
    Preference > Python Interpreter > User Module Reloads (UMR) and uncheck Enable UMR
due to the known issue reported here: https://github.com/spyder-ide/spyder/issues/21269

It cannot be run in Command Line Window within AEDT directly. Minor changes are needed for some lines using Cpython syntax and methods.

Check the example named "AEDT_Reports_HTML_IronPython_InsideAEDT.py" to see what changes are needed in order to run this script inside AEDT.
@author: penghan
"""
...
def write_html(directory, output_file="C:\Workspace\AEDT_Report_Generation\AEDT_HTML_Report_Test.html"):
    """
...
"""
    ...
    for image in image_files:
        ...
        if image.startswith("ModelView"):
            html_content += f'    <h2 id = "top"> Model Views: </h2>\n' # f-string is used in Cpython. In IronPython, change it to string.format(a=a, b=b)
            html_content += f'    <figure>\n'
            html_content += f'    <img src="{image_path}" alt="{image}">\n'
            html_content += f'    <figcaption>Fig.{num_fig} {image.removesuffix(".jpg").removeprefix("ModelView_")} </figcaption>\n'
            html_content += f'    </figure>\n'
    ...
    for image in image_files:
        ...
        if image.startswith("Mesh"):
            html_content += f'    <h2 id = "top"> Mesh Plots: </h2>\n'
            html_content += f'    <figure>\n'
            html_content += f'    <img src="{image_path}" alt="{image}">\n'
            html_content += f'    <figcaption>Fig.{num_fig} {image.removesuffix(".jpg").removeprefix("Mesh_")} </figcaption>\n'
            html_content += f'    </figure>\n'
            html_content += f'    <br><br><br>\n'
            num_fig += 1
        else:
    ...
    for image in image_files:
        ...
        if image.startswith("FieldOverlay"):
            html_content += f'    <h2 id = "top"> Field Overlay Plots: </h2>\n'
            html_content += f'    <figure>\n'
            html_content += f'    <img src="{image_path}" alt="{image}">\n'
            html_content += f'    <figcaption>Fig.{num_fig} {image.removesuffix(".jpg").removeprefix("FieldOverlay_")} </figcaption>\n'
            html_content += f'    </figure>\n'
            html_content += f'    <br><br><br>\n'
            num_fig += 1
        else:
    ...
    for image in image_files:
        ...
        if image.startswith("RecReport"):
            html_content += f'    <h2 id = "top"> Rectangular Reports: </h2>\n'
            html_content += f'    <figure>\n'
            html_content += f'    <img src="{image_path}" alt="{image}">\n'
            html_content += f'    <figcaption>Fig.{num_fig} {image.removesuffix(".jpg").removeprefix("RecReport_")} </figcaption>\n'
            html_content += f'    </figure>\n'
            html_content += f'    <br><br><br>\n'
            num_fig += 1
        else:
